...
def get_model(conf, num_class=10, local_rank=-1):
    ...
    # -ANI ADDING YOLOV8N
        ...
        model = YOLO(
            dataset=conf.get('dataset', 'cifar10'),  # Default to CIFAR-style architecture
            depth=conf.get('depth', 3),              # Number of blocks per stage
            num_classes=num_class,                   # From dataset
            bottleneck_ratio=conf.get('bottleneck_ratio', 0.5)
        )
